...
namespace Sharpshooter.Champions
{
    public static class Kalista
    {
        ...
        public static void Load()
        {
            ...
            SharpShooter.Menu.SubMenu("Misc").AddItem(new MenuItem("kaliscrank", "Use Kaliscrank by DurdenT", true).SetValue(true));
        ...
        static void Obj_AI_Hero_OnProcessSpellCast(Obj_AI_Base sender, GameObjectProcessSpellCastEventArgs args)
        {
            ...
            foreach (var unit in ObjectManager.Get<Obj_AI_Hero>().Where(h => h.IsEnemy && h.IsHPBarRendered))
            {
                ...
                for (int i = 0; i < unit.Buffs.Count(); i++)
                {
                    ...
                    if (soulboundhero.ChampionName == "Blitzcrank" && unit.Buffs[i].Name == "rocketgrab2" && unit.Buffs[i].IsActive && enemy.Count() > 0 && SharpShooter.Menu.Item("kaliscrank", true).GetValue<Boolean>())
                    {
                        R.Cast();
                    }
                }
